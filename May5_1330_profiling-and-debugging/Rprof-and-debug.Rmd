...
```{r, eval=FALSE}
x <- 1:10
c(address(x), refs(x)) # refs = 1
x[1] <- 10
c(address(x), refs(x)) # diff address, refs = 1 (copied)
y <- x
c(address(x), refs(x)) # same address, refs = 2
c(address(y), refs(y)) # same address, refs = 2
y[1] <- 100
c(address(x), refs(x)) # same address, refs = 2
c(address(y), refs(y)) # diff address, refs = 1 (copied)
x[1] <- 100
c(address(x), refs(x)) # diff address, refs = 1 (copied)
```
...
z <- 1:16
z
c(address(z), refs(z)) # refs = 1
names(z) <- letters[1:16]
dim(z) <- c(4,4)
dimnames(z) <- list(letters[1:4], letters[5:8])
z
c(address(z), refs(z)) # same address, refs = 1
```
...
c(address(z), refs(z)) # refs = 1
id(z)
c(address(z), refs(z)) # same address, refs = 2
dim(z) <- NULL
c(address(z), refs(z)) # diff address, refs = 1
```
