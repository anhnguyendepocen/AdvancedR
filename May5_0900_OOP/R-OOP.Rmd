...
Although `data.frame` is an S3 class, the `methods` package also provides an S4 version of many S3 classes, including `data.frame`.
                                   ...
                                   log2 = FALSE)
{
  ...
  new("S4ProteomicsExperiment", object, title=title, log2=log2)
}
```

```{r}
exp2 <- S4ProteomicsExperiment(protein=twin_dia2$protein,
                               feature=twin_dia2$feature,
                               run=twin_dia2$run,
                               intensity=twin_dia2$intensity,
                               label=twin_dia2$label,
...
```{r}
exp2norm <- normalize(exp2, by="heavy")
head(summary(exp2norm)[,,"light"])
plot(exp2norm)
```
