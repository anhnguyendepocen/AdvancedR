...
2. The fitted models were not saved. To extract additional (unplanned) summaries, we need to refit the models. Saving the models in another list is fine, but care is needed to link each model to its corresponding protein.


We now discuss better solution by using:  
...
We learned from the previous section how to obtain grouped summaries with arbitrary operations using `dplyr::group_by()` + `dplyr::do()`. We can now carry out the task with these tools.
...
Note that `dplyr::do()` requires explicit use of the `.`  placeholder, to indicate where the input gets passed on to operate. Multiple rows are generated from each model, each corresponds to a run-level summary of one protein. 
...
List-columns keep together related items in a data frame, thus allowing to perform operations in a data-structure-informed way. Their flexibility makes it convenient to be used as intermediate data structure for a variety of operations which may take different data formats. However, they are generally hard to work with as most R functions are developed for atomic vectors or data frames. To work effectively with list-columns, we need to develope techniques to 
...
We will use **nested data frame**, a particular instance of list-columns, as an example to discuss these techniques. Nested data frame is very useful to carry out the split-apply-combine strategy with arbitrary operations.
...
* The `data` column restores the stratified data corresponding to each protein, in a list (list-column).
...
`unnest()` removes the nested structure by repeating the regular columns for their corresponding element of the list-column.

```{r}
nested_dia %>% unnest(data)
```

Library purrr provides a set of **map functions** to iterate over the list-columns with consistent syntax and return type, which makes it easy to operate on list-columns.
...
Approach 4 with nested data frame allows to explore the data in a principle way.
